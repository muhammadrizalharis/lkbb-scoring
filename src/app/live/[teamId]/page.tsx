...
export default async function LiveTeamDetailPage({
  params,
}: {
  ...
  const gross = categories.reduce((s, c) => s + c.total, 0)

  return (
    <div className="flex min-h-full flex-col">
      <AutoRefresh seconds={20} />
      <PublicHeader name={event.name} host={event.host} liveMode={event.liveMode} backHref="/live" />

      <main className="mx-auto w-full max-w-4xl flex-1 space-y-6 p-4 sm:p-6">
        <div className="rounded-2xl bg-card p-5 shadow-sm ring-1 ring-border">
          <p className="text-sm text-muted-foreground">Rincian Nilai per Butir</p>
          ...
          <div className="mt-3 flex flex-wrap gap-4 text-sm">
            <span>Total nilai: <b className="tabular-nums">{gross}</b></span>
            {penalty > 0 && (
              <span className="text-red-600">Penalti: <b className="tabular-nums">−{penalty}</b></span>
            )}
            <span>Total akhir: <b className="tabular-nums">{gross - penalty}</b></span>
          </div>
        </div>
                ...
                <table className="w-full border-collapse text-sm">
                    ...
                    <tr className="text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                      ...
                      {category.judges.map((j) => (
                        <th key={j.id} className="px-3 py-2 text-right whitespace-nowrap">
                          {j.code}
                          <span className="block text-[10px] font-normal normal-case text-muted-foreground">{j.name}</span>
                        </th>
                      ))}
                    </tr>
                  </thead>
                    ...
                    {category.groups.map((group) => (
                      <Fragment key={group.id}>
                        <tr className="bg-muted/30">
                          <td colSpan={1 + category.judges.length} className="px-4 py-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                        ...
                        {group.criteria.map((c) => (
                          <tr key={c.criterionId} className="align-top">
                            <td className="px-4 py-1.5">
                              {c.name}
                              <span className="ml-1 text-xs text-muted-foreground">(maks {Math.max(...c.options)})</span>
                              {c.notes.map((n, i) =>
                                n ? (
                  ...
                  <tfoot>
                    <tr className="border-t-2 border-border bg-muted/40 font-bold">
                      ...
                      {category.judges.map((j) => (
                        <td key={j.id} className="px-3 py-2 text-right tabular-nums">{j.total}</td>
                      ))}
                    </tr>
                  </tfoot>
                </table>
